...
from rec_animation import rec_animation_process
...
def on_ui_tabs():
    ...
    with gr.Blocks(analytics_enabled=False) as ebs_interface:
        with gr.Row().style(equal_height=False):
                        ...
                        with gr.TabItem('configuration', elem_id='ebs_configuration'):
                            with gr.Tabs(elem_id="ebs_configuration_tab"):
                                with gr.TabItem(label="stage 1",elem_id='ebs_configuration_tab1'):
                                    ...
                                    frame_skip = gr.Number(label='Frame skip', lines=1, value = 2, precision=0, interactive=True)
            ...
            ebs_args = dict(
                fn=wrap_gradio_gpu_call(rec_animation_process),
                inputs=[
                    stage_index,
                    project_code,
                    project_dir,
    ...
    return (ebs_interface, "Rec Animation", "rec_animation_interface"),
