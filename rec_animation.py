...
from animation_stage1 import rec_animation_stage1, ebsynth_utility_stage1_invert
from animation_stage2 import rec_animation_stage2_rename
...
def rec_animation_process(stage_index: int, project_code: str, project_dir: str, original_movie_url: str, original_movie_path: str, frame_skip: int, frame_width: int, frame_height: int, st1_masking_method_index: int, st1_mask_threshold: float, tb_use_fast_mode: bool, tb_use_jit: bool, clipseg_mask_prompt: str, clipseg_exclude_prompt: str, clipseg_mask_threshold: int, clipseg_mask_blur_size: int, clipseg_mask_blur_size2: int, folder_name: str, key_min_gap: int, key_max_gap: int, key_th: float, key_add_last_frame: bool, color_matcher_method: str, st3_5_use_mask: bool, st3_5_use_mask_ref: bool, st3_5_use_mask_org: bool, color_matcher_ref_type: int, color_matcher_ref_image: Image, blend_rate: float, export_type: str, bg_src: str, bg_type: str, mask_blur_size: int, mask_threshold: float, fg_transparency: float, mask_mode: str):
    ...
    frame_path = os.path.join(project_dir, project_code, "video_frame")
    frame_mask_path = os.path.join(project_dir, project_code, "video_mask")

    if is_invert_mask:
        inv_path = os.path.join(project_dir, project_code, "inv")
        ...
        img2img_upscale_key_path = os.path.join(
            inv_path, "img2img_upscale_key")
    else:
        org_key_path = os.path.join(project_dir, "video_key")
        img2img_key_path = os.path.join(project_dir, "img2img_key")
        img2img_upscale_key_path = os.path.join(
            project_dir, "img2img_upscale_key")

    if mask_mode == "None":
        frame_mask_path = ""

    project_args = [project_code, project_dir, original_movie_url, original_movie_path,
                    frame_path, frame_mask_path, org_key_path, img2img_key_path, img2img_upscale_key_path]

    if stage_index == 0:
        rec_animation_stage1(dbg, project_args, frame_skip, frame_width, frame_height, st1_masking_method_index, st1_mask_threshold, tb_use_fast_mode,
                             tb_use_jit, clipseg_mask_prompt, clipseg_exclude_prompt, clipseg_mask_threshold, clipseg_mask_blur_size, clipseg_mask_blur_size2, is_invert_mask)
        if is_invert_mask:
            inv_mask_path = os.path.join(inv_path, "inv_video_mask")
            ebsynth_utility_stage1_invert(dbg, frame_mask_path, inv_mask_path)

    elif stage_index == 1:
        rec_animation_stage2_rename(dbg, project_args, folder_name)
        ...
        dbg.print(
            "3. Fill in the \"Project directory\" field with [" + project_dir + "]")
        dbg.print("4. Select in the \"Mask Mode(Override img2img Mask mode)\" field with [" + (
            "Invert" if is_invert_mask else "Normal") + "]")
        dbg.print(
            "5. I recommend to fill in the \"Width\" field with [" + str(img_width) + "]")
        dbg.print(
            "6. I recommend to fill in the \"Height\" field with [" + str(img_height) + "]")
        dbg.print(
        ...
        ebsynth_utility_stage3_5(dbg, project_args, color_matcher_method, st3_5_use_mask,
                                 st3_5_use_mask_ref, st3_5_use_mask_org, color_matcher_ref_type, color_matcher_ref_image)

    elif stage_index == 4:
        sample_image = glob.glob(os.path.join(frame_path, "*.png"))[0]
        img_height, img_width, _ = cv2.imread(sample_image).shape

        sample_img2img_key = glob.glob(
            os.path.join(img2img_key_path, "*.png"))[0]
        ...
        dbg.print("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
        dbg.print("1. If \"img2img_upscale_key\" directory already exists in the %s, delete it manually before executing." % (
            project_dir))
        ...
        dbg.print(
            "5. Fill in the \"Output directory\" field with [" + img2img_upscale_key_path + "]")
        dbg.print("6. Go to Scale to tab")
        dbg.print(
            "7. Fill in the \"Width\" field with [" + str(img_width) + "]")
        dbg.print(
            "8. Fill in the \"Height\" field with [" + str(img_height) + "]")
        ...
        dbg.print(
            "Open the generated .ebs under %s and press [Run All] button." % (project_dir))
        dbg.print(
            "If ""out-*"" directory already exists in the %s, delete it manually before executing." % (project_dir))
        ...
        ebsynth_utility_stage7(
            dbg, project_args, blend_rate, export_type, is_invert_mask)
    elif stage_index == 8:
        if mask_mode != "Normal":
            dbg.print("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
            dbg.print(
                "Please reset [configuration]->[etc]->[Mask Mode] to Normal.")
            dbg.print("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
            return process_end(dbg, "")
        ebsynth_utility_stage8(dbg, project_args, bg_src, bg_type,
                               mask_blur_size, mask_threshold, fg_transparency, export_type)
    else:
        pass
